...
class TimeDependentAdvectionDiffusion:
    def __init__(self, mesh, Vh, prior, misfit, simulation_times, velocity, u_0, gls_stab=False):
        ...
        phi = None
        # TODO, have L_phi and M_phi here for PG projection speed-up
    ...
    def solveReducedFwd(self, out, x):
        ...
        L_phi = L.matMult(self.phi.mat())
        L_r = dl.PETScMatrix(L_phi.transposeMatMult(L_phi))
        M_phi = dl.as_backend_type(self.M).mat().matMult(self.phi.mat())
        M_r = dl.PETScMatrix(L_phi.transposeMatMult(M_phi))
...
def no_slip_boundary(x, on_boundary):
    ...
    return on_boundary and \
        ((x[0] > dl.DOLFIN_EPS and x[0] < L - dl.DOLFIN_EPS)\
             or (x[1] < dl.DOLFIN_EPS and x[1] > W - dl.DOLFIN_EPS))
